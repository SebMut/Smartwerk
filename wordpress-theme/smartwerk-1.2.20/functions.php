...
/* ==========================================================================
 * SmartWerk 1.2.20 — robust WP3DPrinting asset isolation.
 * The plugin enqueues some assets later than wp_enqueue_scripts priority 999.
 * Filter the final loader tags as a last line of defence on all pages that do
 * not contain the real configurator.
 * ========================================================================== */

function smartwerk_is_wp3dprinting_asset_url(string $html): bool {
    return str_contains($html, '/plugins/3dprint/')
        || str_contains($html, '/plugins/3dprint-round/');
}

function smartwerk_filter_wp3dprinting_script_tag(string $tag, string $handle = ''): string {
    if (smartwerk_is_wp3dprinting_context()) {
        return $tag;
    }

    return smartwerk_is_wp3dprinting_asset_url($tag) ? '' : $tag;
}
add_filter('script_loader_tag', 'smartwerk_filter_wp3dprinting_script_tag', PHP_INT_MAX, 2);

function smartwerk_filter_wp3dprinting_style_tag(string $html, string $handle = ''): string {
    if (smartwerk_is_wp3dprinting_context()) {
        return $html;
    }

    return smartwerk_is_wp3dprinting_asset_url($html) ? '' : $html;
}
add_filter('style_loader_tag', 'smartwerk_filter_wp3dprinting_style_tag', PHP_INT_MAX, 2);
