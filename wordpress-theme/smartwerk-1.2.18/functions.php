...
/**
 * SmartWerk 1.2.9 shop filter controls.
 * Page 1657 renders all WooCommerce products and filters them client-side,
 * keeping category taxonomy and price data in WooCommerce.
 */
function smartwerk_shop_filter_controls(): void {
    if (!is_page(1657)) {
        return;
    }
    ...
    echo '<div class="smartwerk-shop-filters" data-smartwerk-shop-filters>';
    echo '<div class="smartwerk-shop-filter-grid">';

    echo '<label class="smartwerk-shop-filter-field">';
    echo '<span>Produkt suchen</span>';
    echo '<input type="search" data-shop-search placeholder="z. B. Tennis, Ausstecher, Schild" autocomplete="off">';
    echo '</label>';

    echo '<label class="smartwerk-shop-filter-field">';
    echo '<span>Kategorie</span>';
    echo '<select data-shop-category>';
    echo '<option value="">Alle Kategorien</option>';

    foreach ($categories as $category) {
        if ($category->slug === 'unkategorisiert') {
            continue;
        }
    ...
    echo '<label class="smartwerk-shop-filter-field">';
    echo '<span class="smartwerk-shop-price-row"><span>Startpreis bis</span><output data-shop-price-output>250 €</output></span>';
    echo '<input type="range" min="5" max="250" step="5" value="250" data-shop-price>';
    echo '</label>';

    echo '<label class="smartwerk-shop-filter-field">';
    echo '<span>Sortieren</span>';
    echo '<select data-shop-sort>';
    echo '<option value="default">Neueste zuerst</option>';
    echo '<option value="price-asc">Preis aufsteigend</option>';
    echo '<option value="price-desc">Preis absteigend</option>';
    echo '<option value="name-asc">Name A–Z</option>';
    echo '</select>';
    echo '</label>';

    echo '</div>';

    echo '<div class="smartwerk-shop-filter-footer">';
    echo '<label class="smartwerk-shop-personalized">';
    echo '<input type="checkbox" data-shop-personalized>';
    echo '<span>Nur personalisierbare Produkte</span>';
    echo '</label>';

    echo '<div class="smartwerk-shop-filter-summary">';
    echo '<span><strong data-shop-result-count>0</strong> Produkte</span>';
    echo '<button class="smartwerk-shop-reset" type="button" data-shop-reset>Filter zurücksetzen</button>';
    echo '</div>';

    echo '</div>';
    echo '</div>';
    echo '<div class="smartwerk-shop-no-results" data-shop-no-results hidden>Keine Produkte passen zu diesen Filtern.</div>';
}
add_action('woocommerce_before_shop_loop', 'smartwerk_shop_filter_controls', 5);
 ...
 */
function smartwerk_strip_legacy_page_shells(string $content): string {
    if (is_admin() || !is_page([1636, 1638, 1640, 1642, 1657])) {
        return $content;
    }

    $content = preg_replace('/<!--\s*SmartWerk Design\s*-->\s*/i', '', $content) ?: $content;
    $content = preg_replace(
        '#<link[^>]+href=["\'][^"\']*/wp-content/themes/kiosko-child/[^"\']+["\'][^>]*>\s*#i',
        '',
        $content
    ) ?: $content;

    $content = preg_replace(
        '#<header\s+class=["\']sw-(?:header|global-header)["\'][^>]*>[\s\S]*?</header>\s*#i',
        '',
        $content
    ) ?: $content;

    $content = preg_replace(
        '#<footer\s+class=["\']sw-(?:footer|global-footer)["\'][^>]*>[\s\S]*?</footer>\s*#i',
        '',
        $content
    ) ?: $content;

    $content = preg_replace(
        '#<script[^>]*>[\s\S]*?/wp-json/wc/store/v1/cart[\s\S]*?</script>\s*#i',
        '',
        $content
    ) ?: $content;

    return str_replace('/smartwerk-startseite-neu/', '/', $content);
}
add_filter('the_content', 'smartwerk_strip_legacy_page_shells', 8);
